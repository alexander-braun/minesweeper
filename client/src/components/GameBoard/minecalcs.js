const MineCalcs = (() => {
    ...
    const floodFill = (posx, posy) => {
      ...
      // If has mine around return without new floodfill but mark as revealed
        ...
        floodFill(posx, posy + 1) // nach rechts
        floodFill(posx, posy - 1) // nach links

        floodFill(posx - 1, posy - 1) // nach unten links
        floodFill(posx - 1, posy + 1) // nach unten rechts

        floodFill(posx + 1, posy + 1) // nach oben rechts
        floodFill(posx + 1, posy - 1) // nach oben links
      }
    }
